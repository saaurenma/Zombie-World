...
public class Zombie extends ZombieActor {
	 ...
	 */
	private Behaviour[] behaviours = { new AttackBehaviour(ZombieCapability.ALIVE), new PickUpBehaviour(),
			new HuntBehaviour(Human.class, 10), new WanderBehaviour() };

	/**
	 * List of behaviours the Zombie is capable of performing with damaged or
	 * removed legs.
	 */
	private Behaviour[] leglessBehaviours = { new AttackBehaviour(ZombieCapability.ALIVE), new PickUpBehaviour(), };
	...
	@Override
	/**
	 * Unique IntrinsicWeapon getter for Zombie. Gives a chance of returning a Bite
	 * rather than a Punch. The bite chance increases as the Zombie loses arms.
	 */
	...
	/**
	 * Determines which limb the Zombie loses, based on its remaining limbs. Creates
	 * and drops a ZombieArm or ZombieLeg object, depending on the limb lost.
	 * 
	 * @param map the map where the current Zombie is
	 */
	...
	/**
	 * Drops the Zombie's weapons when it loses an arm. 50% chance if the Zombie has
	 * one arm, 100% if it has none.
	 * 
	 * @param map the map where the current Zombie is
	 */
